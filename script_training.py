...
###### data
# video_dataset = data.VideoDataset(tr_data_idx_dir, tr_data_frame_dir, transform=frame_trans, type=None)
# video_dataset = data.VideoDataset(data_root, transform=frame_trans)
...
dataset2d = Dataset2d(data_root, transform=frame_trans)
tr_data_loader = DataLoader(dataset2d,
                            batch_size=batch_size_in,
                            shuffle=True,
                            num_workers=opt.NumWorker,
                            )
...
with open(f"./Feature_Train/Feature_{opt.Dataset}.pkl", "wb") as f:
